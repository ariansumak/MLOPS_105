...
This is the report template for the exam. Please only remove the text formatted with three dashes in front and behind
like:
...
* [x] Remember to either fill out the `requirements.txt`/`requirements_dev.txt` files or keep your
...
* [x] Add a continuous workflow that triggers when changes to the model registry are made (M19)
...
* **dockerfiles/** - Contains the Dockerfiles for training and for the FastAPI service that exposes health and prediction endpoints and loads model + config.
...
* **reports/** - Contains sample images of the dataset and statistics like class distribution across the different splits of the dataset.

We have slightly deviated from the template, having removed the following directories:

* **data/** - The original data/ directory was intentionally removed from Git tracking and replaced by DVC-based data versioning.

### Question 6

> **Did you implement any rules for code quality and format? What about typing and documentation? Additionally,**
> **explain with your own words why these concepts matter in larger projects.**
> Answer:
...
The total code coverage of our code is 41%, which includes all our source code. We are far from 100% coverage, but even if we were that wouldn't mean it's error free. A lot of errors stem from permissions and missing or misconfigured environment variables in cloud deployments. Most of the tests use dummy data to validate functions inside each file, plus a few API tests using Locust.

### Question 9

> **Did your workflow include using branches and pull requests? If yes, explain how. If not, explain how branches and**
> **pull requests can help improve version control.**
> Answer:
...
> **Discuss your continuous integration setup. What kind of continuous integration are you running (unit testing,**
...
An example Dockerfile can be seen here: `https://github.com/ariansumak/MLOPS_105/blob/main/dockerfiles/train.dockerfile`
...
We used Cloud Storage buckets to store our data for the data drifting tasks and for data versioning control. Artifact Registry was used to store the built Docker images. We used Cloud Build and Triggers for automatic builds of Docker images on every push to the main branch, using a workflow in GitHub Actions and a cloudbuild.yaml file. We used Vertex AI to run our Docker image for training. Finally, Cloud Run was used for deployment of backend and data drifting services.
...
We never used Compute Engine directly. For training we set up a vertex_ai_train.yaml file that would configure the VMs and for deployment we used Cloud Run.
...
We trained our model only using Vertex AI, because we found it to be easier during the exercises. Once we had set up a Docker image that would have everything necessary to run our training script, including wandb logging, all we had to do was write a yaml file for Vertex AI, which was pretty similar to the one given in exercises. We just injected our wandb API key as a secret from GCP and it was set to go. It worked almost immediately, so we had no reason to try out Compute Engine.
...
critical for maintaining model performance over time and deciding when retraining is needed. The GCS logs can be buggy and do not work perfectly, so this is not a finished product.
...
We implemented a frontend because it made it easier to navigate the inference of the model and allowed a preview of the data. It was also fairly simple to do due to some previous experience in that area.
...
There were a lot of issues with the GCP functionality, due to the lack of logging and high complexity of some tasks. There were merge issues as many branches overlapped, and multiple times we had to resolve conflicts that sometimes broke the code. Wandb also created issues when using artifacts to load the model checkpoint, and not everyone could access or gain permissions to the data there. Permissions were generally a big issue in GCP since some team members could not be granted access to the needed bucket.

When images are built on the cloud or using the Docker image, it takes a lot of memory and time, so it was hard to debug issues and keep focus on such tasks.
...
Student s252552 focused mostly on organizational tasks, Docker images, and the report. He also created and maintained the repository structure. Student s254629 created the CLI, helped with DVC, and created the monitoring. Student s253470 oversaw most of the GCP cloud operations and created the Hydra configurations. He also created the dataloader for the initial machine learning model. Student s253819 oversaw the API processes and created the machine learning model training and evaluation scripts, connected initially with wandb, and created the version control setups. He also focused on the drift detection algorithms.

We all used generative AI in the form of Codex and ChatGPT/Gemini. It was mostly used for improving code readability and when someone was stuck at a task.
