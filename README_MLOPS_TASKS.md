...
## Python Entry Points (uv run)

High-level shortcuts defined in `pyproject.toml`:
...
## Automation Tasks (invoke)

Task runner commands located in `tasks.py`:
...
| **Full Train**   | `uv run invoke train --epochs 5`                    | Runs `dvc pull` then starts training.                 |
| **Dev Servers**  | `uv run invoke dev`                                 | Starts both API (port 8000) and Frontend (port 8001). |

> **Tip:** Use `uvi` as alias for `uv run invoke` to save keystrokes.
